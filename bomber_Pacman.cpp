...
//const float DISPLACEMENT = 60.0f; //speed of the BomberMan
//float bomberman_x = 78.0f; // x coordinate of the BomberMan character
//float bomberman_y = 90.0f; // y coordinate of the BomberMan character

float currentbombposition_x;
float currentbombposition_y;
...
void GameDisplay()/**/{
	...
	glClearColor(0/*Red Component*/, 0,	//148.0/255/*Green Component*/,
		0.00/*Blue Component*/, 0 /*Alpha component*/); // Red==Green==Blue==1 --> White Colour
	...
	b->GetInitDinkyPosition(x, y);
	cout << x << "  y= " << y << endl << flush;
	DrawEnemy(x + 720, y - 60, DARK_ORANGE, 0.8 * b->GetCellSize(), 0.8 * b->GetCellSize());
	...
	DrawString(x, y, Num2Str(b->GetScore()), colors[5]);
		glPopMatrix();

	b->BombPlacement(bombStatus);
	...
	if (bombStatus == true) {
		...
		if (timelapse < 2)
		{
			...
			currentbombposition_x = x;
			currentbombposition_y = y;
		}
		
		DrawCircle(currentbombposition_x, currentbombposition_y, 15, colors[WHITE_SMOKE]); // To Draw a bomb. You will provide your own x and y relative to Bombermans position
		
		if ( !(timelapse <= 5)) // Controls max distance that could be travelled
		{
			bombStatus = false;
			DrawCircle(currentbombposition_x + 60, currentbombposition_y, 15, colors[RED]);
			DrawCircle(currentbombposition_x - 60, currentbombposition_y, 15, colors[RED]);
			DrawCircle(currentbombposition_x, currentbombposition_y + 60, 15, colors[RED]);
			DrawCircle(currentbombposition_x, currentbombposition_y - 60, 15, colors[RED]);
			timelapse = 0;
		}
			timelapse++;
	...
	glutSwapBuffers(); // do not modify this line.. this will redraw the scene
